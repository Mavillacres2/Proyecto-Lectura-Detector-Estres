...
export default function Login() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Opcional: Alerta de éxito (puedes quitarla si prefieres fluidez)
      // alert("Login exitoso");

      // --- AQUÍ VA LA REDIRECCIÓN INTELIGENTE (Dentro del try) ---
      // Verificamos el rol que nos devolvió el backend
      if (res.data.role === "admin") {
        nav("/admin"); // Si es admin, va al dashboard
      } else {
        nav("/detector"); // Si es estudiante, va al test
      }
      // -----------------------------------------------------------

    } catch (err: any) {
      console.error(err); // Útil para ver errores en consola (F12)
      // Mensaje un poco más descriptivo
      alert(err.response?.data?.detail || "Error al conectar. Si es la primera vez, el servidor puede estar despertando.");
    } finally {
